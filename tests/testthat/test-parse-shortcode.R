...
test_that("C++ string shortcodes parser - basic functionality", {
  ...
  expect_equal(
    check_string_shortcodes_parser("Hello {{< video url >}} world"),
    list(rmd_shortcode("video", "url", 6, 17))
  )
  
  # Test multiple shortcodes
  expect_equal(
    check_string_shortcodes_parser("{{< func1 >}} and {{< func2 arg >}}"),
    list(
  ...
  # Test no shortcodes
  expect_equal(
    check_string_shortcodes_parser("Hello world with no shortcodes"),
    list()
  )
  
  # Test shortcode with no arguments
  expect_equal(
    check_string_shortcodes_parser("{{< pagebreak >}}"),
...
test_that("C++ string shortcodes parser - argument parsing", {
  ...
  expect_equal(
    check_string_shortcodes_parser("{{< video arg1 arg2 arg3 >}}"),
    list(rmd_shortcode("video", c("arg1", "arg2", "arg3"), 0, 28))
  )
  
  # Test quoted arguments
  expect_equal(
    check_string_shortcodes_parser("{{< video \"quoted arg\" >}}"),
    list(rmd_shortcode("video", "\"quoted arg\"", 0, 26))
  )
  
  # Test single quoted arguments
  expect_equal(
    check_string_shortcodes_parser("{{< kbd key='Ctrl+C' >}}"),
    list(rmd_shortcode("kbd", "key='Ctrl+C'", 0, 24))
  )
  
  # Test key=value arguments
  expect_equal(
    check_string_shortcodes_parser("{{< video url=https://example.com title=\"My Video\" >}}"),
    list(rmd_shortcode("video", c("url=https://example.com", "title=\"My Video\""), 0, 54))
  )
  
  # Test mixed argument types
  expect_equal(
    check_string_shortcodes_parser("{{< include file.txt lines=10 echo=true >}}"),
...
test_that("C++ string shortcodes parser - multiline strings", {
  ...
  expect_equal(
    check_string_shortcodes_parser("{{< video https://example.com/video.mp4
  ...
  expect_equal(
    check_string_shortcodes_parser("Start {{< pagebreak >}}
  Middle text here
  {{< video url >}} more text
  ...
  expect_equal(
    check_string_shortcodes_parser("Line 1\n{{< shortcode arg >}}\nLine 3"),
...
test_that("C++ string shortcodes parser - complex quoted arguments", {
  ...
  # Test arguments with spaces in quotes
  expect_equal(
    check_string_shortcodes_parser("{{< video \"file with spaces.mp4\" >}}"),
    list(rmd_shortcode("video", "\"file with spaces.mp4\"", 0, 36))
  )
  
  # Test escaped quotes in arguments
  expect_equal(
    check_string_shortcodes_parser("{{< kbd key=\"\\\"quoted\\\"\" >}}"),
    list(rmd_shortcode("kbd", "key=\"\\\"quoted\\\"\"", 0, 28))
  )
  
  # Test mixed quotes
  expect_equal(
    check_string_shortcodes_parser("{{< func arg1=\"value1\" arg2='value2' >}}"),
    list(rmd_shortcode("func", c("arg1=\"value1\"", "arg2='value2'"), 0, 40))
  )
  
  # Test unterminated quote (should handle gracefully)
  expect_error(
    check_string_shortcodes_parser("{{< func \"unterminated >}}"),
...
test_that("C++ string shortcodes parser - real-world examples", {
  ...
  expect_equal(
    check_string_shortcodes_parser("{{< video https://www.youtube.com/embed/wo9vZccmqwc
  ...
  expect_equal(
    check_string_shortcodes_parser("# My Document
  ...
  expect_equal(
    check_string_shortcodes_parser("{{< div .class #id data-attr=value >}}"),
